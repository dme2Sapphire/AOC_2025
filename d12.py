...
# print(shape_tnum)
# print(ctree_num)
naive_valid = [((ctree['shape'][0]//3)*(ctree['shape'][1]//3) >=
                sum(ctree['giftlist'])) for ctree in ctrees]  # 460/1000
...
# print(shape_area_c)
naive_invalid = [(ctree['shape'][0]*ctree['shape'][1] < sum([a*b for a,
                  b in zip(shape_area_c, ctree['giftlist'])])) for ctree in ctrees]
# print(sum(naive_invalid))


# print(sum(naive_valid)+sum(naive_invalid) == ctree_num) # WTF DO YOU MEAN THAT THIS IS TRUE??????

if (sum(naive_valid)+sum(naive_invalid) == ctree_num):
    print(f"Q1:{sum(naive_valid)}")
    print(f"Q2:Merry (Early) Christmas!")